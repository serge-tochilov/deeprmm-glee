"""Prior-positive pooling and longitudinal seller control for GLEE Persuasion v2.6."""
...
from .glee_persuasion_policy_v2_4 import PersuasionSellerPolicyConfig, canonical_seller_message
from .glee_persuasion_twin_v2 import PersuasionContext


SCHEMA_VERSION = 1
POLICY_VERSION = "persuasion-longitudinal-seller-v2.6"
...
def seller_longitudinal_control(context: PersuasionContext, *, current_quality: str | None, buyer_response_forecasts: Mapping[str, object], config: PersuasionSellerPolicyConfig | None = None) -> dict[str, object]:
    """Pool positive signals when buying is profitable at the public prior, otherwise preserve the v2.4 longitudinal policy."""
    ...
    normalized_prior_surplus = context.normalized_prior_surplus
    pooling_eligible = context.seller_knows_buyer_values is True and normalized_prior_surplus is not None and normalized_prior_surplus > selected.deception_advantage_margin
    result: dict[str, object] = {
        ...
        "normalized_prior_expected_surplus": round(normalized_prior_surplus, 6) if normalized_prior_surplus is not None else None,
        "prior_positive_pooling_margin_ratio": selected.deception_advantage_margin,
        "prior_positive_pooling_eligible": pooling_eligible,
        "selected_signal_polarity": None,
        "selection_reason": "current quality unavailable",
        "authority": "none",
        "config": asdict(selected),
        "boundary": "Prior-positive pooling applies only when the seller sees buyer values and expected buyer surplus at the public quality prior exceeds the conservative price-normalized margin. The unchanged v1 parameter seller_deception_advantage_margin supplies both this margin and the late-harvest margin.",
    }
    if current_quality not in {"high", "low"}:
        return result
    if pooling_eligible:
        result.update({"selected_signal_polarity": "positive", "selection_reason": "buyer prior expected surplus remains positive under an uninformative pooled signal", "authority": "semantic-authority"})
        return result
